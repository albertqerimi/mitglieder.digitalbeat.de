...
class Request
{
    ...
    public function call($call)
    {
        ...
        if ($authArgsCount > 1) {
            $headers['Authorization'] = 'Basic ' . base64_encode(join(':', $this->auth));
        } else {
            $headers['Authorization'] = 'Bearer ' . $this->auth[0];
        }

        $payload['headers'] = array_merge($this->headers, $headers);
        ...
        if ($call) {
            try {
                $options = [
                    ...
                    'sslverify' => false,
                    'timeout' => 30,
                    'headers' => array_merge($this->headers, $payload['headers']),
                ];
                if ($this->method == 'POST') {
                    $options['body'] = json_encode($this->body ? $this->body : []);
                }
