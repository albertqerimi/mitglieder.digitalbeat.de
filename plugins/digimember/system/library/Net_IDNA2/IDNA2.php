...
class ncore_Net_IDNA2
{
    ...
    private function _nameprep($input)
    {
        ...
        foreach ($input as $v) {
            ...
            if (in_array($v, self::$_np_prohibit) || in_array($v, self::$_general_prohibited)) {
                throw new InvalidArgumentException('Prohibited input U+' . sprintf('%08X', $v));
            }
    ...
    private static function _showHex($input, $include_bit = false)
    {
        foreach ($input as $k => $v) {
            ...
            if ($include_bit) {
                echo ' (', ncore_Net_IDNA2::_showBitmask($v), ')';
            }
    ...
    {
        return new ncore_Net_IDNA2($params);
    }
    // }}}
     ...
     * @access public
     */
    static function singleton($params = array())
    {
        static $instances;
        if (!isset($instances)) {
            $instances = array();
        ...
        if (!isset($instances[$signature])) {
            $instances[$signature] = ncore_Net_IDNA2::getInstance($params);
        }
